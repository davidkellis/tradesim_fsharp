...
let ``correlation returns the sample correlation coefficient (Pearson's r coefficient)`` () = 
  ...
  Sample.Decimal.correlation xs ys |> Decimal.roundTo 10 |> should equal 0.9925662059M
  ...
  Sample.Decimal.correlation xs ys |> Decimal.roundTo 3 |> should equal 0.880M

  xs <- [12m; 14m; 15m; 16m; 18m]
  ys <- [32m; 35m; 45m; 50m; 65m]
  Sample.Decimal.correlation xs ys |> Decimal.roundTo 3 |> should equal 0.968M

  xs <- [350000m; 399765m; 429000m; 435000m; 433000m]
  ys <- [32m; 35m; 45m; 50m; 65m]
  Sample.Decimal.correlation xs ys |> Decimal.roundTo 3 |> should equal 0.772M

[<Test>]
let ``mean returns sample mean`` () =
  let mutable xs = [12m; 14m; 15m; 16m; 18m]
  Sample.Decimal.Seq.mean xs |> should equal 15m

  xs <- [32m; 35m; 45m; 50m; 65m]
  Sample.Decimal.Seq.mean xs |> should equal 45.4m

  xs <- [350000m; 399765m; 429000m; 435000m; 433000m]
  Sample.Decimal.Seq.mean xs |> should equal 409353m

[<Test>]
let ``stdDev computes sample standard deviation`` () =
  let mutable xs = [12m; 14m; 15m; 16m; 18m]
  Sample.Decimal.stdDev xs |> Decimal.roundTo 3 |> should equal 2.236m

  xs <- [32m; 35m; 45m; 50m; 65m]
  Sample.Decimal.stdDev xs |> Decimal.roundTo 3 |> should equal 13.164m

  xs <- [350000m; 399765m; 429000m; 435000m; 433000m]
  Sample.Decimal.stdDev xs |> Decimal.roundTo 3 |> should equal 36116.693m
...
let ``simpleOls performs a linear regression`` () =
  ...
  let result = Sample.Decimal.simpleOls xs ys
...
let ``ols performs a linear regression`` () =
  ...
  let result = Sample.Double.ols xs ys
...
let ``linearModel estimates a linear equation that fits the given data points`` () =
  ...
  let mutable ys: Vector<double> = vector [f 152.15 8.21    
                                           ...
                                           f 98.2   198.2   
                                           f -18.5  17.686  ]
  let f' = Sample.Double.linearModel xs ys
...
let ``quantilesR8 should calculate the nth quantile`` () =
  ...
  Sample.Decimal.Seq.percentiles [0m; 10m; 20m; 25m; 30m; 40m; 50m; 60m; 70m; 80m; 90m; 100m] xs 
