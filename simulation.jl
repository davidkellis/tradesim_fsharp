...
function compute_dist_stats(distribution, expected_mean, return_array = Array(Float64, 6), print = true)
  # compute statistics of sampling distribution
  q005, q5, q995 = round(quantile(distribution, [0.005, 0.5, 0.995]), 3)
  ...
  return_array[3] = range_of_99th_percentile_of_samp_dist
  return_array[4] = q005
  return_array[5] = q5
  return_array[6] = q995
  return_array
...
function compute_samp_dist_stats(samp_dist)
  ...
  "mean=$mean_of_samp_dist   std=$std_of_samp_dist   0.5%=$q005  1%=$q01  5%=$q05  10%=$q1  20%=$q2  30%=$q3  40%=$q4  50%=$q5  60%=$q6  70%=$q7  80%=$q8  90%=$q9  95%=$q95  99%=$q99  99.5%=$q995"
end
...
function main()
  ...
  n_return_observations = 52
  n_annualized_returns = 100000
  n_samples = 1000
  ...
  for i in 1:n_iterations
    ...
    annualized_returns = build_distribution(return_observations, n_annualized_returns, n_periods_per_year, (sample) -> prod(sample), annualized_returns)
    
    stats = compute_dist_stats(annualized_returns, 1)
    ...
    # mu, sigma, range, q005, q5, q995 = compute_dist_stats(samp_dist_of_mean_annual_return, 1)
  end
...
function main2()
  ...
  for i in 1:100
    ...
    compute_dist_stats(samp_dist, 1)
  end
end
